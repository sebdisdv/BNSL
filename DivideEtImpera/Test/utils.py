...
def getAdjMat(n, xt):
    ...
    return (
        torch.cat(
            [
                torch.cat(
                    [torch.zeros(n - 1).view(-1, 1), xt[:narcs].view(-1, n)], dim=-1
                ).view(-1),
                torch.zeros(1),
            ]
        )
...
def get_variables_combinations(
    n: int, sub_problem_cardinality: int = 3
) -> Tuple[Tuple[int]]:
...
def get_variables_combinations_with_replacement(
    n: int, sub_problem_cardinality: int = 3
) -> Tuple[Tuple[int]]:
    if sub_problem_cardinality < 3:
        raise ValueError
...
def get_variables_permutations(
    n: int, sub_problem_cardinality: int = 3
) -> Tuple[Tuple[int]]:
    if sub_problem_cardinality < 3:
        raise ValueError
...
def get_variable_disposition_with_repetition(
    n: int, sub_problem_cardinality: int = 3
) -> Tuple[Tuple[int]]:
    if sub_problem_cardinality < 3:
        raise ValueError
...
def recompose(AdjMatrixSubproblems, N) -> Tuple[List[List[int]], Dict]:
    ...
    for edge in edgeCount:
        ...
        try:
            rev_edge = edge[::-1]
            if (
                edgeCount[edge] > edgeCount[rev_edge]
                and edgeCount[edge] >= 1
                and (edgeCount[edge] - wrongEdgeCount[edge] > 0)
            ):
